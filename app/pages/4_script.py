...
st.set_page_config(page_title="脚本生成", page_icon="📝")
st.title("脚本生成")
...
influencer_options = {
    row["influencer_id"]: f"{row['influencer_id']} · {row['name']}" for row in influencers
}
influencer_id = st.selectbox(
    "选择达人",
    options=list(influencer_options.keys()),
    format_func=lambda iid: influencer_options[iid],
)
